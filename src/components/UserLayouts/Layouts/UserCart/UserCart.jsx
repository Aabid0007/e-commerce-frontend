...
import { createCheckoutSession, deleteCart, getAllCarts, updateQuantity } from '../../../../Redux/Slices/User.Slice';
...
const UserCart = () => {
    ...
    const { userCarts, error, totalQuantity } = useSelector((state) => state.user);
    ...
    const handleCheckout = (cartItems) => {
        ...
        dispatch(createCheckoutSession({ cartItems: userCarts, userId, totalPrice }))
    }
                    ...
                    <div className='cartSection'>
                                            ...
                                            cart.items.map((item, index) => (
                                                <tr key={`${cart._id}_${index}`}>
                                                    <td className='cartCard'>
                                                        <img className='cartImage' src={`http://localhost:5001/${item.productDetails.images[0]}`} alt='Product' />
                                                    ...
                                                    <td className='tableTd'>
                                                        {item.productDetails && item.productDetails.price ? item.productDetails.price : '-'}
                                                    </td>
                                                    <td>
                                                        <button onClick={() => handleCartRemove(item.productDetails._id)}>remove</button>
                                                    </td>
                                                </tr>
                                            ))
                                        ))}
                        ...
                        {totalQuantity ? <div className='productCheckoutSection'>
                            <div className='totalPrice'>
                                ...
                                <button className='productCheckout' onClick={() => handleCheckout({ userCarts })}>
